...
def get_news_item(start_date, end_date):
    ...
    while int(cursor) > int(finished_cursor):
        ...
        CommonUtil.save_to_file('../files/wallstreetcn_%s_%s.csv' % (start_date, end_date), file_content)
        file_content = ''
        # 无下一页数据时退出循环
        if cursor == '':
            break
...
logger = LogHelper.Logger("../logs/wallstreetcn_crawler.log")
startDate = ''
endDate = ''
...
if len(sys.argv) > 2:
    for num in range(1, 3):
        ...
        if CommonUtil.check_date(sys.argv[num]):
            if num == 1:
                startDate = sys.argv[num]
            if num == 2:
                endDate = sys.argv[num]
        else:
            startDate = CommonUtil.get_today()
            endDate = startDate
